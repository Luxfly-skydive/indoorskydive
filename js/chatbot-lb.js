(function () {
    ...
    <div id="lf-chat-window" role="dialog" aria-label="Luxfly Chat">
      <div id="lf-chat-header">
        ...
        <div id="lf-chat-header-info">
          ...
          <div id="lf-chat-header-status">Online · Ëmmer do fir Iech</div>
        </div>
      ...
      <div id="lf-chat-quick">
        <button class="lf-quick-btn" data-q="Wat kascht et?">💰 Präisser</button>
        <button class="lf-quick-btn" data-q="Wéini sidd Dir op?">🕐 Aarengszeiten</button>
        <button class="lf-quick-btn" data-q="A sécher fir Kanner?">👦 Kanner</button>
        <button class="lf-quick-btn" data-q="Wéi kann ech buchen?">🎟️ Buchen</button>
        <button class="lf-quick-btn" data-q="Wou sidd Dir?">📍 Standuert</button>
        <button class="lf-quick-btn" data-q="Wat empfeelt Dir?">⭐ Empfehlung</button>
      </div>
      <form id="lf-chat-form" autocomplete="off">
        <textarea id="lf-chat-input" placeholder="Stellt eng Fro…" rows="1"></textarea>
  ...
  async function sendMessage(text) {
    ...
    try {
      ...
      removeTyping();
      addMsg('Verbindungsproblem. Schreift un **hello@luxfly.eu** oder WhatsApp **+32 2 320 55 09** 😊', 'bot');
    }
    input.focus();
  }
  ...
  function openChat() {
    ...
    if (!greeted) {
      greeted = true;
      setTimeout(() => { addMsg('Moien! 👋 Ech si Zoom, den LUXFLY Gebuertsdag-Assistent. Freet mech alles iwwer eis Pakete fir Kanner a Jugendlecher — Präisser, HighFly, Buchung, Standuert a méi!', 'bot'); }, 300);
    }
  ...
  bubble.id = 'lf-chat-bubble'; bubble.textContent = 'Eng Fro? Hei froen!';
