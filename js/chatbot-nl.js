(function () {
    ...
    <div id="lf-chat-window" role="dialog" aria-label="Luxfly Chat">
      <div id="lf-chat-header">
        ...
        <div id="lf-chat-header-info">
          ...
          <div id="lf-chat-header-status">Online · Altijd beschikbaar</div>
        </div>
      ...
      <div id="lf-chat-quick">
        <button class="lf-quick-btn" data-q="Wat zijn de prijzen?">💰 Prijzen</button>
        <button class="lf-quick-btn" data-q="Wanneer zijn jullie open?">🕐 Openingstijden</button>
        ...
        <button class="lf-quick-btn" data-q="Waar zijn jullie gevestigd?">📍 Locatie</button>
        <button class="lf-quick-btn" data-q="Welk pakket raden jullie aan?">⭐ Aanbeveling</button>
      </div>
      <form id="lf-chat-form" autocomplete="off">
        <textarea id="lf-chat-input" placeholder="Stel een vraag…" rows="1"></textarea>
  ...
  async function sendMessage(text) {
    ...
    try {
      ...
      removeTyping();
      addMsg('Verbindingsprobleem. Mail **hello@luxfly.eu** of WhatsApp **+32 2 320 55 09** 😊', 'bot');
    }
    input.focus();
  }
  ...
  function openChat() {
    ...
    if (!greeted) {
      greeted = true;
      setTimeout(() => { addMsg('Hallo! 👋 Ik ben Zoom, de LUXFLY verjaardagsassistent. Stel me vragen over onze paketten voor kinderen en tieners — prijzen, HighFly, boeking, locatie en meer!', 'bot'); }, 300);
    }
  ...
  bubble.id = 'lf-chat-bubble'; bubble.textContent = 'Heeft u een vraag? Stel ze hier!';
